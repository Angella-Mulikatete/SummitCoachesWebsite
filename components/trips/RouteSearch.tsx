...
export default function RouteSearch({ onSearch, className }: RouteSearchProps) {
    ...
    // Fetch routes WITHOUT filters to populate dropdowns
    // Don't pass active or has_trips to get all possible routes
    const { data: routes, isLoading } = useRoutes({});

    // Extract unique origins and destinations
    const origins = Array.from(
        new Set(routes?.map((r: any) => r.origin).filter(Boolean))
    ).sort();
    
    const destinations = Array.from(
        new Set(routes?.map((r: any) => r.destination).filter(Boolean))
    ).sort();

    const handleSearch = () => {
        console.log('Search clicked with:', { origin, destination, date });
        onSearch({
            origin: origin && origin !== 'all' && origin !== '' ? origin : undefined,
            destination: destination && destination !== 'all' && destination !== '' ? destination : undefined,
            date: date,
        });
    };
